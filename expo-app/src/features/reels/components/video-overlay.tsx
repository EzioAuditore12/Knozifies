...
import { useSeek } from '../contexts/seek.context';
import { useTabBarLayout } from '../contexts/tab-bar-layout.context';
import {
...
const VideoOverlay = ({
  ...
  const [stableTabBarHeight, setStableTabBarHeight] =
    useState<number>(fallbackTabBarHeight);
  ...
  const displayProgress = seekingProgress !== null ? seekingProgress : progress;
    ...
    >
      ...
      <View style={[styles.overlayRight, { bottom: rightColumnBottom }]}>
        ...
        {PERFORMANCE_MONITOR_ENABLED && (
          <TouchableOpacity style={styles.iconButton} onPress={toggleMetrics}>
            <Ionicons
              name="stats-chart"
              size={RIGHT_ICON_SIZE}
              color={`rgba(255,255,255,${RIGHT_ICON_OPACITY})`}
        ...
        >
          <View
            style={[
              styles.seekBarFill,
              {
                width: `${Math.min(1, Math.max(0, displayProgress)) * 100}%`,
              },
            ]}
          />
        </Animated.View>
      ...
      >
        <Text style={styles.seekTimerText}>
          {formatSeekTime(displayProgress * (duration > 0 ? duration : 0))}
          {' / '}
