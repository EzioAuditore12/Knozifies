...
const VideoViewComponent = React.memo(
  function VideoViewComponent({
    ...
    useEffect(() => {
      ...
      if (shouldPreload || isActive) {
        if (!currentUri || currentUri !== video.url) {
          if (ttffStartTimeRef.current === null && !ttffMeasuredRef.current) {
    ...
    useEffect(() => {
      ...
      const shouldPlay = isActive && !userPaused && appState === 'active';
    ...
    const duration = useMemo(() => player?.duration ?? 0, [player?.duration]);
