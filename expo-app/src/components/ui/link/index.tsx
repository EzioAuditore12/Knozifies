...
// Setup interop for ExpoLink and ExpoLink.Text
...
const Link = React.forwardRef<LinkProps, ILinkProps>(
...
type ILinkTextProps = React.ComponentProps<typeof Text> &
  VariantProps<typeof linkTextStyle> & { className?: string };

const LinkText = React.forwardRef<Text, ILinkTextProps>(
  ({ className, size = 'md', children, ...props }, ref) => (
    <Text
      ref={ref}
      {...props}
      className={linkTextStyle({ class: className, size })}
    >
      {children}
    </Text>
  ),
